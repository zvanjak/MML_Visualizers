    ...
    for (auto& entry : legend->entries_) {
        if (entry.checkbox == checkbox) {
            ...
            if (legend->visibilityCallback_) {
                legend->visibilityCallback_(entry.curveIndex, visible);
            }
            break;
        }
    }
...
void LegendWidget::AddItem(const std::string& title, const CurveColor& color, int curveIndex, bool visible) {
...
bool LegendWidget::IsVisible(int curveIndex) const {
    for (const auto& entry : entries_) {
        if (entry.curveIndex == curveIndex) {
...
void LegendWidget::SetVisible(int curveIndex, bool visible) {
    for (auto& entry : entries_) {
        if (entry.curveIndex == curveIndex && entry.checkbox) {
