...
MainWindow::MainWindow(int argc, char** argv) : fileIndex_(0) {
    // Create main window
    window_ = new Fl_Window(1200, 700, "MML Real Function Visualizer (FLTK)");
...
void MainWindow::UpdateLegend() {
    ...
    for (const auto& func : functions) {
        if (func->GetDimension() == 1) {
            // Single function
            legendWidget_->AddItem(func->GetTitle(), GraphWidget::GetColor(colorIndex));
            colorIndex++;
        } else {
            // Multi-function
            for (int i = 0; i < func->GetDimension(); ++i) {
                legendWidget_->AddItem(func->GetFunctionTitle(i), GraphWidget::GetColor(colorIndex));
                colorIndex++;
            }
        }
    }
