# PowerShell Script to visualize Particle 2D Simulation data files
# Using FLTK MML_ParticleVisualizer2D_FLTK

# Configuration
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$VisualizerPath = Join-Path $ScriptDir "MML_ParticleVisualizer2D_FLTK.exe"
...
Write-Host "  Particle 2D Visualizer Runner" -ForegroundColor Green
Write-Host "          (FLTK Version)" -ForegroundColor Magenta
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
if (-not (Test-Path $VisualizerPath)) {
    ...
    Write-Host "Please ensure the FLTK visualizer is built and deployed." -ForegroundColor Yellow
    exit 1
}
...
# Get all .txt files from the Basic directory
